...
const (
	...
	kekInfo   = "condor-ccb-session-cache-dek-v1"
)

// SigningKey is one HTCondor signing key used as a key-encryption key. ID is the
// key name (e.g. "POOL"); Material is the raw (unscrambled) key bytes.
...
// newEnvelope generates a fresh random DEK.
...
var errNoKey = errors.New("ccb: no available signing key can decrypt the session cache")
...
// seal encrypts plaintext with the DEK, returning a fresh nonce and ciphertext.
